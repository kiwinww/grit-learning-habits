...
$credential = Get-Credential -UserName "root" -Message "Enter the Aliyun ECS password for this preview deployment."
...
try {
  ...
  $payload | node (Join-Path $root "scripts\deploy-preview-password.mjs")
  if ($LASTEXITCODE -ne 0) { throw "Preview deployment failed with exit code $LASTEXITCODE" }
}
finally {
